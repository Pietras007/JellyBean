...
namespace Geometric2.ModelGeneration
{
    public class Cube : Element
    {
        ...
        public override void CreateGlElement(Shader _shader, Shader _shaderLight)
        {
            ...
            //cubePoints = new float[ConfigurationData.BezierBoxDivisions * ConfigurationData.BezierBoxDivisions * 8];
        ...
        public override void RenderGlElement(Shader _shader, Shader _shaderLight, Vector3 rotationCentre, GlobalPhysicsData globalPhysicsData)
        {
            if (globalPhysicsData.displayBezierCube)
            {
                ...
                //cubePoints = new float[] { -1.5f,-1.5f,-1.5f, -1,0,0,0.5f,0.5f,
                //-1.5f,1.5f,-1.5f, -1,0,0,0.5f,0.5f,
                //-1.5f,-1.5f,1.5f, -1,0,0,0.5f,0.5f};
                //indices = new uint[] { 0, 1, 2 };
                RecalculateGeometry();
                _shaderLight.Use();
        ...
        private void GeneratePatches(GlobalPhysicsData globalPhysicsData)
        {
            ...
            foreach (var patch in patches)
            {
                ...
                {
                    indicesList.Add(commonIndices[i] + idx * maxNoIndice + 1);
                }
            ...
            var tempCubePoints = GenerateOnlyPoints(points);
            //if (cubePoints == null)
            {
                cubePoints = tempCubePoints;
            }
        }

        private float[] GenerateOnlyPoints(List<Vector3> points)
        {
